...
exports.resendTicketsEmail = exports.sendTicketsEmail = exports.generateTicketsForOrder = void 0;
exports.dispatchTicketsEmailForOrder = dispatchTicketsEmailForOrder;
const https_1 = require("firebase-functions/v2/https");
const admin = require("firebase-admin");
const emailProvider_1 = require("./services/emailProvider");
const ticketFunctions_1 = require("./ticketFunctions");
 ...
 */
async function dispatchTicketsEmailForOrder(orderId, operatorUid, operatorName, options) {
    ...
    if (orderData.status !== 'PAID') {
        throw new https_1.HttpsError('failed-precondition', `No se pueden enviar entradas para órdenes con estado ${orderData.status}. La orden debe estar pagada (PAID).`);
    }
    ...
    if (existingTicketsQuery.empty) {
        if (options?.isResend) {
            throw new https_1.HttpsError('failed-precondition', 'No existen boletos generados para esta orden. Emita los boletos primero.');
        }
        // Generación idempotente de boletos
        tickets = await (0, ticketFunctions_1.issueTicketsForPaidOrder)(orderId, operatorUid, operatorName);
    }
    else {
        tickets = existingTicketsQuery.docs.map((d) => d.data());
    }
    if (!tickets || tickets.length === 0) {
        throw new https_1.HttpsError('failed-precondition', `La orden ${orderId} no cuenta con entradas disponibles para envío.`);
    }
    ...
    const emailTicketItems = tickets.map((t) => ({
        ticketId: t.id,
    ...
    const sendResult = await emailProvider_1.emailProvider.sendTicketsEmail(emailPayload);
 ...
 */
exports.generateTicketsForOrder = (0, https_1.onCall)({
    cors: true,
    maxInstances: 10
}, async (request) => {
    if (!request.auth) {
    ...
    const tickets = await (0, ticketFunctions_1.issueTicketsForPaidOrder)(orderId, operator.uid, operator.displayName);
    return {
        success: true,
        orderId,
...
//# sourceMappingURL=emailDispatchFunctions.js.map
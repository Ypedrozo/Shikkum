  ...
  async (request: CallableRequest<CreateOrderData>) => {
    ...
    if (!request.auth) {
      throw new HttpsError(
        'unauthenticated',
        'Debe estar autenticado para registrar órdenes comerciales.'
      );
    }
    ...
    if (!requestId || typeof requestId !== 'string' || requestId.trim().length < 5) {
      throw new HttpsError('invalid-argument', 'Se requiere un identificador de idempotencia (requestId) válido.');
    }
    if (!customerId || typeof customerId !== 'string') {
      throw new HttpsError('invalid-argument', 'El identificador del cliente (customerId) es obligatorio.');
    }
    if (!eventId || typeof eventId !== 'string') {
      throw new HttpsError('invalid-argument', 'El identificador del evento (eventId) es obligatorio.');
    }
    if (!Array.isArray(attendees) || attendees.length === 0) {
      throw new HttpsError('invalid-argument', 'La orden debe contener al menos un asistente válido.');
    }
    ...
    if (userData.role !== 'admin' && userData.role !== 'cashier') {
      throw new HttpsError('permission-denied', 'Solo Administrador y Cobranzas tienen autorización para crear órdenes.');
    }
    ...
    if (!customerDoc.exists) {
      throw new HttpsError('not-found', 'El cliente especificado no existe en el sistema.');
    }
    const customerData = customerDoc.data();
    if (!customerData || !customerData.isActive) {
      throw new HttpsError('failed-precondition', 'El cliente seleccionado está inactivo. No se permiten órdenes para clientes inactivos.');
    }
    ...
    if (!eventDoc.exists) {
      throw new HttpsError('not-found', 'El evento especificado no existe.');
    }
    const eventData = eventDoc.data();
    if (!eventData) {
      throw new HttpsError('not-found', 'Información de evento no disponible.');
    }
    if (eventData.status !== 'ACTIVE') {
      throw new HttpsError(
        'failed-precondition',
        `El evento no está disponible para ventas. Estado actual: ${eventData.status}. Solo se admiten órdenes para eventos ACTIVE.`
      );
    }
    ...
    for (let i = 0; i < attendees.length; i++) {
      ...
      if (!att.fullName || typeof att.fullName !== 'string' || att.fullName.trim() === '') {
        throw new HttpsError('invalid-argument', `El asistente #${i + 1} debe tener un nombre completo válido.`);
      }
      if (typeof att.ageAtPurchase !== 'number' || isNaN(att.ageAtPurchase) || att.ageAtPurchase < 0) {
        throw new HttpsError('invalid-argument', `La edad del asistente "${att.fullName}" debe ser un número entero mayor o igual a 0.`);
      }
    }
    ...
    if (rulesSnapshot.empty) {
      throw new HttpsError('failed-precondition', 'El evento no tiene reglas de precios activas configuradas.');
    }

    const availableRules: PriceRuleRecord[] = [];
    rulesSnapshot.forEach((doc: admin.firestore.QueryDocumentSnapshot) => {
      availableRules.push({
        id: doc.id,
        ...(doc.data() as Omit<PriceRuleRecord, 'id'>)
      });
    });
    ...
    const evaluatedAttendees: Array<{
      id: string;
      fullName: string;
      ageAtPurchase: number;
      isCommunityMemberAtPurchase: boolean;

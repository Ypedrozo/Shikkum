 ...
 */
export async function issueTicketsForPaidOrder(
  orderId: string,
  operatorUid: string,
  operatorName: string
) {
  ...
  if (orderData.status !== 'PAID') {
    throw new HttpsError(
      'failed-precondition',
      `Solo se pueden emitir tickets para órdenes con estado PAID (estado actual: ${orderData.status}).`
    );
  }
  ...
  if (attendeesSnapshot.empty) {
    throw new HttpsError('failed-precondition', `La orden ${orderId} no contiene asistentes registrados.`);
  }
  ...
  return await db.runTransaction(async (transaction: admin.firestore.Transaction) => {
    ...
    if (freshOrderData.status !== 'PAID') {
      throw new HttpsError(
        'failed-precondition',
        `Solo se pueden emitir tickets para órdenes con estado PAID (estado actual: ${freshOrderData.status}).`
      );
    }
 ...
 */
export const validateTicketAccess = onCall<{
  qrToken?: string;
  ticketCode?: string;
  gateId?: string;
  eventId?: string;
  ...
  async (request: CallableRequest<{
    ...
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Debe estar autenticado para validar acceso en puerta.');
    }
    ...
    if (!cleanEventId) {
      throw new HttpsError(
        'invalid-argument',
        'Debe especificar un evento específico para el control de acceso en puerta.'
      );
    }
    ...
    if (!trimmedToken && !trimmedCode) {
      throw new HttpsError(
        'invalid-argument',
        'Se requiere el token del código QR (qrToken) o el código del ticket (ticketCode).'
      );
    }
    ...
    let ticketDoc: admin.firestore.DocumentSnapshot;
    ...
    return await db.runTransaction(async (transaction: admin.firestore.Transaction) => {
      ...
      if (!freshDoc.exists) {
        throw new HttpsError('not-found', 'El boleto no existe.');
      }
 ...
 */
export const resendTicketEmail = onCall<{
  ticketId: string;
  ...
  async (request: CallableRequest<{
    ...
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Debe estar autenticado para reenviar boletos.');
    }
    ...
    if (!ticketId || typeof ticketId !== 'string') {
      throw new HttpsError('invalid-argument', 'El identificador del boleto es obligatorio.');
    }
    ...
    if (!recipient) {
      throw new HttpsError('invalid-argument', 'No se ha definido un correo de destino.');
    }
...
export const cancelTicket = onCall<{
  ...
  async (request: CallableRequest<{
    ...
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Debe estar autenticado para cancelar boletos.');
    }
    ...
    if (!reason || typeof reason !== 'string' || reason.trim().length < 5) {
      throw new HttpsError(
        'invalid-argument',
        'Debe proporcionar un motivo de cancelación claro y descriptivo (mínimo 5 caracteres).'
      );
    }
    ...
    return await db.runTransaction(async (transaction: admin.firestore.Transaction) => {
      ...
      if (ticket.status === 'USED') {
        throw new HttpsError(
          'failed-precondition',
          'No es posible cancelar un boleto que ya fue utilizado para ingresar al evento.'
        );
      }

      if (ticket.status === 'CANCELLED') {
        throw new HttpsError('failed-precondition', 'El boleto ya se encuentra cancelado.');
      }
 ...
 */
export const issueTicketsForOrder = onCall<{ orderId: string }>(
  {
    cors: true,
    maxInstances: 10
  },
